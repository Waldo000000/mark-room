...
import {
  scoreMarkRoomAnswer,
  type MarkRoomQuizQuestion,
  type MarkRoomQuizResult,
} from '@/src/domain/quiz/mark-room';

type MarkRoomQuizProps = {
  question: MarkRoomQuizQuestion;
  reviewHref: string;
  teachingText?: string;
};

export function MarkRoomQuiz({
  question,
  reviewHref,
  teachingText,
}: MarkRoomQuizProps) {
  const [selectedBoatId, setSelectedBoatId] = useState('');
  const [result, setResult] = useState<MarkRoomQuizResult | null>(null);

  function submitAnswer() {
    if (!selectedBoatId) return;
    setResult(scoreMarkRoomAnswer(question, selectedBoatId));
  }
  ...
  return (
    <section aria-labelledby="mark-room-question" data-testid="mark-room-quiz">
      <p className="text-sm font-semibold uppercase text-muted-foreground">
        Mark-room check
      </p>
      <h2 className="mt-2 text-xl font-semibold" id="mark-room-question">
        {question.prompt}
      </h2>
      ...
      {result ? (
        <div
          ...
          data-testid="mark-room-quiz-feedback"
        >
